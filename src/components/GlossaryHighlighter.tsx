...
export function GlossaryTerm({ term, children }: GlossaryTermProps) {
  ...
  return (
    <Link
      href={`/${locale}/glossary#${termId}`}
      className="glossary-term-link inline-flex items-center gap-0.5"
      title={locale === 'de' ? `Begriff im Glossar ansehen: ${term}` : `View in glossary: ${term}`}
    >
      {children || term}
      <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full transition-colors opacity-60 hover:opacity-100 shrink-0">
        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
        </svg>
      </span>
